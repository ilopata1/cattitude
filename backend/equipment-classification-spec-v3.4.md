# Equipment classification specification — v3.4
...
**v3.4** adds Stage 1.6 derived actions, a one-shot evidence repair pass,

`fewshot_leakage` / `evidence_incomplete` blocking flags, and golden-comparison

ordering that skips when `needs_rextraction` is true. Rules apply to **every**
...
**Ordering (extract path):** Stage 1 → Stage 1.5 validate → if

`evidence_incomplete`, one repair call → re-validate → Stage 1.6 derive →

final validate. Golden live comparison (`compare_smartsolar_scratch`) runs only

on the **post-validation** profile and **skips** when `needs_rextraction` is
...
- Strict JSON Schema with `additionalProperties: false` on every object
  ...
  `json_schema` / `strict`). Extraction schema does **not** include Stage 1.6

  `source` / `derived_from` (those are code-added).
...
### Calibration isolation (v3.4)



Calibration examples concern **OTHER** devices / situations. Match field

decisions (context, audience, dependency shape). Do **not** copy example

action strings into the device under extraction unless that wording appears

in **this** device's excerpts. Example C frames emergency / operator fields

inline without a copyable stock action phrase.
...
Evidence rule (v3.2+): prefer coverage via the priority list (cap 1–8);
...
Runs on **every** profile between Stage 1 and Stage 1.6. Annotates:
...
| Calibration stock action string not grounded in excerpts | `fewshot_leakage` | blocking |

| True `data_roles` / `requires_devices` entry without evidence | `evidence_incomplete` | blocking |
...
### Evidence repair pass (v3.4)



When `evidence_incomplete` is present, run **one** targeted repair LLM call

(`interaction_profile_repair.py`): profile + routed excerpts + the named missing

fields → emit evidence entries for exactly those fields → merge → re-validate.

Cap: one attempt. Remaining `evidence_incomplete` stays blocking for human review.



`needs_rextraction` is true if any **blocking** flag is present. Blocking

profiles must not enter Stage 2 (re-extraction queue). Live golden comparison

must skip when `needs_rextraction` is true.
...
- Corrected SmartSolar (asserts **zero** `evidence_verbatim`, derived/extracted

  consult-error-codes emergency action, `has_emergency_procedure: true`):
...
## Stage 1.6 — Derived actions (v3.4)



Module: `interaction_profile_derive.py` (pure code).



1. Tag every existing operator action with `source: "extracted"` when unset.

2. If `safety_role.has_emergency_procedure == true` **and** any

   `evidence[].manual_section` matches `/error|fault|alarm|troubleshoot/i`,
...

   {
     ...
     "action": "consult error codes and alarms",
     ...
     "derived_from": "evidence[N]"

   }

   ```

   unless a semantically similar action already exists.



Does not invent fault tables — only promotes evidence that already cites them.

Verify: `scripts/verify_interaction_profile_derive.py`.
...
| 3.4 | Stage 1.6 derived actions; `fewshot_leakage` + `evidence_incomplete` + one-shot repair; golden compare skips on `needs_rextraction`; calibration isolation preamble |
