"""Draft Air Conditioning Stage 4 section for Outremer / Supernova (frozen tip v4.46).

Persists ``ac_section_inputs.json`` beside the draft for input-set review.

Review markdown includes the Equipment Locations table above narrative prose.

Usage (from backend/):
  python scripts/draft_ac_section.py
"""
...
from guide_section_ac import compose_ac_section, evaluate_ac_draft
...
OUT_JSON = OUT_DIR / "ac_section_draft_v4.json"
OUT_MD = OUT_DIR / "ac_section_draft_v4.md"
OUT_INPUTS = OUT_DIR / "ac_section_inputs.json"
...
def main() -> int:
    ...
    inputs = assemble_section_inputs(graph, "ac", equipment_doc=equipment_doc)
    try:
        composed = compose_ac_section(
            graph=graph,
            profiles=profiles,
            equipment_doc=equipment_doc,
    ...
    expect_path = (
        _BACKEND / "tests" / "fixtures" / "ac_section_v4_expectations.json"
    )
    expected = _load(expect_path) if expect_path.is_file() else {}
    evaluation = evaluate_ac_draft(
    ...
    payload = {
        ...
        "config_placeholder_ids": composed.get("config_placeholder_ids"),
        "ac_key": composed.get("ac_key"),
    }
    OUT_JSON.write_text(
    ...
    print("\n--- excluded (context_shaping) ---")
    for e in inputs.get("candidates_excluded") or []:
        print(f"  {e.get('device_key')} ({e.get('candidacy')})")
    print("\n--- fact_queries ---")
