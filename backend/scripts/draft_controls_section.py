"""Draft Controls & Monitoring Stage 4 section for Outremer / Supernova.

Usage (from backend/):
  python scripts/draft_controls_section.py
"""
...
OUT_JSON = OUT_DIR / "controls_section_draft_v4.json"
OUT_MD = OUT_DIR / "controls_section_draft_v4.md"
OUT_INPUTS = OUT_DIR / "controls_section_inputs.json"
...
def main() -> int:
    ...
    expect_path = (
        _BACKEND / "tests" / "fixtures" / "controls_section_v4_expectations.json"
    )
    ...
    payload = {
        ...
        "gated_off_pages": composed.get("gated_off_pages"),
    ...
    print("\n--- excluded candidates ---")
    for ex in inputs.get("candidates_excluded") or []:
        print(f"  {ex['device_key']}: {ex.get('note', '')[:80]}")
    print("\n--- evaluation ---")
