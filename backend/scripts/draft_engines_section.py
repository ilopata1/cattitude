"""Draft Engines Stage 4 section for Outremer / Supernova.

Usage (from backend/):
  python scripts/draft_engines_section.py
"""
...
from guide_section_engines import compose_engines_section, evaluate_engines_draft
...
OUT_JSON = OUT_DIR / "engines_section_draft_v4.json"
OUT_MD = OUT_DIR / "engines_section_draft_v4.md"
OUT_INPUTS = OUT_DIR / "engines_section_inputs.json"
...
def main() -> int:
    ...
    inputs = assemble_section_inputs(graph, "engines", equipment_doc=equipment_doc)
    try:
        composed = compose_engines_section(
            graph=graph,
            profiles=profiles,
            equipment_doc=equipment_doc,
    ...
    expect_path = (
        _BACKEND / "tests" / "fixtures" / "engines_section_v4_expectations.json"
    )
    expected = _load(expect_path) if expect_path.is_file() else {}
    evaluation = evaluate_engines_draft(
