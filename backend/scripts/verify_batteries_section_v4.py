"""Verify Batteries section inputs + composition (criteria xxvi–xxxi).

Also asserts Solar leaf input regression (assignment-only closure).

Usage (from backend/):
  python scripts/verify_batteries_section_v4.py
"""

from __future__ import annotations

import json
import sys
from pathlib import Path

_BACKEND = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(_BACKEND))

from guide_section_batteries import compose_batteries_section, evaluate_batteries_draft
from section_inputs import assemble_section_inputs, keys_at_depth
from system_graph import build_vessel_graph

OUTREMER = _BACKEND / "fixtures" / "pipeline" / "outremer"
EXPECT = _BACKEND / "tests" / "fixtures" / "batteries_section_v4_expectations.json"


def _load(path: Path):
    return json.loads(path.read_text(encoding="utf-8"))


def main() -> int:
    failures: list[str] = []
    expect = _load(EXPECT)
    equipment_doc = _load(OUTREMER / "equipment.json")
    profiles = _load(OUTREMER / "profiles.json")
    graph = build_vessel_graph(
        list(equipment_doc["equipment"]),
        profiles,
        relations=list(equipment_doc.get("relations") or []),
        equipment_doc=equipment_doc,
        vessel_artifact_facts=equipment_doc.get("vessel_artifact_facts"),
    )

    solar_keys = tuple(expect["solar_leaf_member_keys"])
    solar_inputs = assemble_section_inputs(
        graph,
        "batteries",
        equipment_doc=equipment_doc,
        member_keys=solar_keys,
    )
    solar_got = sorted(c["device_key"] for c in solar_inputs["contributors"])
    solar_exp = sorted(solar_keys)
    if solar_got != solar_exp:
        failures.append(
            f"solar leaf inputs {solar_got} != assignment-only {solar_exp}"
        )
    else:
        print("OK — solar leaf input set == assignment-only members")

    inputs = assemble_section_inputs(
        graph, "batteries", equipment_doc=equipment_doc
    )
    got = {c["device_key"]: c["depth"] for c in inputs["contributors"]}
    exp = {
        c["device_key"]: c["depth"]
        for c in expect["expected_inputs"]["contributors"]
    }
    if got != exp:
        failures.append(f"batteries inputs mismatch got={got} expected={exp}")
    else:
        print("OK — batteries input set matches fixture (xxvi)")

    composed = compose_batteries_section(
        graph=graph,
        profiles=profiles,
        equipment_doc=equipment_doc,
        section_inputs=inputs,
    )
    evaluation = evaluate_batteries_draft(
        composed, expected_inputs=expect["expected_inputs"]
    )

    draft = composed["draft_markdown"]
    for s in expect["forbidden_prose_substrings"]:
        if s.lower() in draft.lower():
            failures.append(f"forbidden prose: {s!r}")

    for s in expect.get("required_prose_substrings") or []:
        if s.lower() not in draft.lower():
            failures.append(f"missing required prose: {s!r}")

    got_queries = {
        str(q.get("id"))
        for q in (composed.get("fact_queries") or [])
        if isinstance(q, dict)
    }
    for qid in expect.get("expected_fact_query_ids") or []:
        if qid not in got_queries:
            failures.append(f"missing fact_query id={qid!r}")
    for qid in expect.get("forbidden_fact_query_ids") or []:
        if qid in got_queries:
            failures.append(f"unexpected fact_query id={qid!r}")

    for soft in expect.get("style_deictics_soft") or []:
        if soft.lower() in draft.lower():
            print(f"STYLE — deictic still present: {soft!r}")

    authorial = [
        w
        for w in (evaluation.get("style_warnings") or [])
        if w.get("code") == "authorial_xref_voice"
    ]
    if authorial:
        failures.append(f"authorial xref voice still present: {authorial}")

    link_targets = {
        str(link.get("target_id"))
        for link in (composed.get("guide_links") or [])
        if link.get("target_kind") == "system"
    }
    for tid in expect.get("expected_guide_link_targets") or []:
        if tid not in link_targets:
            failures.append(f"missing guide_link target_id={tid!r}")

    prefix = expect.get("required_block_order_prefix") or []
    got_blocks = list(composed.get("block_order") or [])
    if got_blocks[: len(prefix)] != prefix[: len(got_blocks)] or any(
        b not in got_blocks for b in prefix
    ):
        failures.append(f"block order {got_blocks} != prefix {prefix}")

    if not evaluation.get("pass"):
        failures.append(f"evaluation failed: {evaluation.get('notes')}")

    if failures:
        print("FAIL:")
        for f in failures:
            print(" -", f)
        print(draft)
        print(json.dumps(evaluation, indent=2))
        return 1

    print("OK — batteries v4.21 composition + xxvi–xli")
    print("style_warnings:", json.dumps(evaluation.get("style_warnings") or [], indent=2))
    print("wisdom_slot:", json.dumps(composed.get("wisdom_slot") or evaluation.get("global_composition", {}).get("wisdom_slot"), indent=2))
    print("guide_links:", json.dumps(composed.get("guide_links") or [], indent=2))
    print("full:", keys_at_depth(inputs, "full"))
    print("provenance:", keys_at_depth(inputs, "provenance"))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
